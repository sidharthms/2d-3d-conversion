...
local function createModel(opt)
   ...
   local gpu1upsample = nn.ParallelTable()
   for i=1, 4 do
      ...
      if opt.magicpony_feature_upsample_kernel > 0 then
         local kW = opt.magicpony_feature_upsample_kernel
         convUpsampleNet:add(Convolution(outChannels, outChannels * 4^(i-1), kW,kW,1,1,(kW-1)/2,(kW-1)/2))
         convUpsampleNet:add(nn.PixelShuffle(2^(i-1)))
      else
      ...
      gpu1upsample:add(convUpsampleNet)
   end
   local gpu1seq = nn.ParallelTable():add(nn.Identity()):add(gpu1upsample)

   if opt.kaimint_init then
      kaimingInit(gpu1seq)
   end
   local gpu1net = nn.GPU(gpu1seq, gpu1, gpu2):cuda()

   cutorch.setDevice(gpu2)
   -- local inputTable = nn.Identity()()
   -- local img = nn.SelectTable(1)(inputTable)
   -- local upsampled = nn.SelectTable(2)(inputTable)

   local dispMapNet = nn.Sequential()
   -- dispMapNet:add(nn.PrintSize('dispMapNet input'))
   -- dispMapNet:add(Break('dispMapNet input'))
   ...
   local gpu2net = nn.GPU(gpu2seq, gpu2):cuda()
